...
impl File {
    ...
    pub fn add_to_fd_table(self, flags: usize) -> LinuxResult<i32> {
...
impl Clone for FdTable {
    fn clone(&self) -> Self {
        ...
        Self {
            files: new_files,
            cloexec_bitmap: self.cloexec_bitmap.clone(),
        }
    }
}
...
impl FdTable {
    ...
    /// Initializes the file descriptor table and the cloexec bitmap.
    pub fn new() -> Self {
        Self {
    ...
    pub fn add(&mut self, file: Arc<dyn FileLike>, flags: usize) -> Option<usize> {
        if let Some(fd) = self.files.add(file) {
            debug_assert!(self.cloexec_bitmap.get(fd) == false);
    ...
    pub fn dup(&mut self, fd: usize) -> LinuxResult<usize> {
        ...
        debug_assert!(self.cloexec_bitmap.get(new_fd) == false);
        Ok(new_fd)
    }
...
impl FileSystem {
    ...
    pub fn close_all_files(&mut self) {
        for fd in 0..self.fd_table.files.capacity() {
            if let Some(_) = self.fd_table.files.get(fd) {
